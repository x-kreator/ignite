    ...
    public void testAddForRecycle() throws Exception {
        try {
            ...
            try {
                try {
                    while (i < 198) {
                        cache.put(i, bytes(10000 /* (rnd.nextInt(100) + 1)*/));
                        i++;
                    }
                    ...
                    log.warning("first IOOME: ", e);

                    while (true) {
                        cache.put(i, bytes(10));
                        i++;

                        log.info("*** second series put success: " + i);
                    }
                }
            }
            catch (Exception e) {
                log.warning("second IOOME: ", e);
            }
            finally {
                ...
                for (int j = 0; j < 256; j++) {
                    ...
                    if (buckets[j] != null) {
                        ...
                        for (PagesList.Stripe s : buckets[j]) {
                            //s.tailId
                        }
                    }
                ...
                log.info("stripe count: " + stripeCnt);
